            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-success shadow-success border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize ps-3">Pengaturan Deadline</h6>
                    </div>
                </div>
...
document.getElementById('tahunSelect').addEventListener('change', function() {
    ...
    fetch(`?controller=deadlinePerangkat&method=getByTahun&id_tahun_pelajaran=${idTahun}`)
        .then(res => res.json())
        .then(res => {
    ...
    onchange="saveDeadline(${item.id_deadline !== null ? item.id_deadline : 'null'}, '${item.jenis_perangkat}', this.value)" />
...
function saveDeadline(id, jenis, tanggal) {
    ...
    let url = isUpdate ? 
        `?controller=deadlinePerangkat&method=update` : 
        `?controller=deadlinePerangkat&method=storeSingle`;

    let bodyData = isUpdate ? 
        `id_deadline=${id}&tanggal_deadline=${tanggal}` : 
        `id_tahun_pelajaran=${idTahun}&jenis_perangkat=${encodeURIComponent(jenis)}&tanggal_deadline=${tanggal}`;
    ...
    .then(res => {
        if (!res.success) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
                text: res.message || 'Gagal menyimpan deadline',
                timer: 2000,
            ...
            Swal.fire({
                icon: 'success',
                title: isUpdate ? 'Berhasil Diperbarui' : 'Berhasil Ditambahkan',
                text: 'Deadline berhasil disimpan',
                timer: 1500,
                showConfirmButton: false
            });
            if (!isUpdate) {
